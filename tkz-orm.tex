...
Object types are drawn as circles. The object's type name is written as node text inside. \emph{Entity types} use solid border lines and \emph{value types} use dashed border lines. The minimal size of an object is set to 6mm$\times$6mm. This package provides the following styles for entities and values:
...
\begin{stylekey}{orm-compact}
  If the circles are too big for your document, you can typeset object types as rounded rectangles (as was the default in \tkzorm\ 0.1.3 and before), by using \texttt{orm-compact} instead of \texttt{orm}.
\begin{codeexample}[width=3cm]
\begin{tikzpicture}[orm-compact]
  \node[entity] (P) at (0,0) {Person};
  \node[value]  (N) at (1.5,0) {Name};
...
\begin{description}
  ...
  \item[0.1.4, January 15, 2016]
    Power types, sequence types, \texttt{orm-compact} added. Default style of objects changed from rounded rectangle to circle.
